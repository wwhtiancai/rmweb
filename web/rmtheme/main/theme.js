function panel(){
	var m="";
	var height = 360;
	var containers = parent.document.getElementsByTagName("frame");
	if (containers && containers.length > 0) {
		for(var i=0;i<containers.length;i++){
			if(containers[i].contentWindow == window){
				height = containers[i].clientHeight - 60;
			}
		}
	}
...
function block(){
	...
	for(var i=0;i<v.size();i++){
		...
		t+='<fieldset style="width:'+w+';text-align:left;border:2px groove white;">';
		...
		t+='<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center"><tr><td align="center">';
		t+=h;
		t+='</td></tr></table>';
