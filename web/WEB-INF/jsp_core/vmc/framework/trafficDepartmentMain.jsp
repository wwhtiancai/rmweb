	...
	<table border="0" cellspacing="1" cellpadding="0" class="query" width="100%">
		...
		<tr>
			<td colspan="6" class="submit">
				<input type="button" value="添加" class="button_query" onClick="add()">&nbsp;
...
<div id="result">
	...
	<table border="0" cellspacing="1" cellpadding="0" class="list">
		...
		<tr class="out" ondblclick="showdetail(${current.glbm})" onMouseOver="this.className='over'" onMouseOut="this.className='out'" style="cursor:pointer">
...
<script type="text/javascript">
	...
	function query_cmd(){
		...
		$("#myform").attr("action","<c:url value='/department.vmc?method=listTraffic'/>");
		closes();
		$("#myform").submit();
	}
	function doChecking(){
		//object:PLS_DEPARTMENT
		//regulation:glbm(等长(12))
		//regulation:bmmc(超长(64))
		if(!checkPrecision($("#glbm"),"12","管理部门")) return false;
		if(!checkLength($("#bmmc"),"64","部门名称")) return false;
		return true;
	}
	function showdetail(glbm){
		openwin("<c:url value='/department.vmc'/>?method=detailTraffic&glbm="+glbm,"department");
	}
	function add() {
		openwin("<c:url value='/department.vmc'/>?method=addTraffic");
	}
	//-->
</script>
</body>
