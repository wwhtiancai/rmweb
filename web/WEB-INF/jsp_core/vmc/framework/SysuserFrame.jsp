...
<body id="sysUserManagement">
<div id="panel" style="display:none">
	<div id="paneltitle">用户管理</div>
	<c:if test="${gnid!=null&&gnidsize>0}">
		<table border="0" cellspacing="0" cellpadding="0" align="center" class="tag_head_table">
			<tr id="tag_console">
				...
				<c:set var="gnidcount" value="1" />
				<c:forEach items="${gnid}" var="current">
					<td class="tag_head_close" align="center" onClick="changepanel(<c:out value='${gnidcount}'/>)" id="tagitem<c:out value='${gnidcount}'/>">
						<c:out value='${current.mc}' />
					</td>
					<c:set var="gnidcount" value="${gnidcount+1}" />
				</c:forEach>
				<td class="tag_head_bg">
					<span class="s1">&nbsp;</span>
				</td>
			</tr>
		</table>
		...
		<c:forEach items="${gnid}" var="current">
			<table border="0" cellspacing="0" cellpadding="0" class="tag_body_table" id="tagpanel<c:out value='${gnidcount}'/>" style="display: none;">
				<tr>
					<td>
						<iframe id="tagframe<c:out value='${gnidcount}'/>" src="" width="100%" marginwidth="0" marginheight="0" align="top" scrolling="no" frameborder="0"></iframe>
					</td>
				</tr>
			</table>
			<c:set var="gnidcount" value="${gnidcount+1}" />
