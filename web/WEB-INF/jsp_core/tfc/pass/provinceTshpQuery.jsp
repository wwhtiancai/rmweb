...
//		getTollgate();
	...
	function getListHtml() {
		...
		curHtml += '<tr><td colspan="8" class="page">共' + totalCount
				+ '条&nbsp;共' + pageCount + '页&nbsp;第' + curPage + '页 &nbsp;';
	...
	function showDetail(gcxh, pageNum, listNum) {
		detailUrl = "<c:url value='tshpQuery.tfc?method=showProvinceTshpDetail&gcxh='/>"
	...
	function gotoPage(idx) {
		...
		setQueryingHtml();
		curUrl = "<c:url value='tshpQuery.tfc?method=queryProvinceTshp&goto=true&page='/>"
				+ idx + "&model=" + queryModel;
...
<body>
	<div id="tag_body" class="tag_body">
		<form name="myform" id="myform"
			action="<c:url value='tshpQuery.tfc?method=queryProvinceTshp'/>"
			method="post">
					...
					<td width="24%" class="body" colspan="5">
						<ul style="width:100%;">
							<c:forEach var="current" items="${csysList}">
								<li style="float:left;width:55px;"><input type="checkbox"
									id="csys<c:out value='${current.dmz}'/>" name="csys"
									value="<c:out value="${current.dmz}"/>"> <c:out
										value="${current.dmz}" />:<c:out value="${current.dmsm1}" />
								</li>
							</c:forEach>
						</ul></td>
				</tr>
