...
function getPass(idx){
	...
	closes();
	if(kl == 'local'){
		curUrl = "<c:url value='passQuery.tfc?method=doPassLocalDetail&page='/>" + curPage + "&idx=" + idx + "&model=<c:out value='${model}'/>";
	}else if(kl == 'province'){
		curUrl = "<c:url value='passQuery.tfc?method=doPassProvinceDetail&page='/>" + curPage + "&idx=" + idx + "&model=<c:out value='${model}'/>";
	}else if(kl == 'proroaming'){
		curUrl = "<c:url value='passQuery.tfc?method=doPassProRoamingDetail&page='/>" + curPage + "&idx=" + idx + "&model=<c:out value='${model}'/>";
	}else if(kl == 'national'){
		curUrl = "<c:url value='passQuery.tfc?method=doPassNationalDetail&page='/>" + curPage + "&idx=" + idx + "&model=<c:out value='${model}'/>";
	}else{
		alert('查询异常！');
		return;
	}
