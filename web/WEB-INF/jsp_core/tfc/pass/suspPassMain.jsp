...
function changepanel(idx){
	for (var i=1;i<=gnids.length;i++){
		$("#tagitem"+i).attr("className","tag_headx2_close");
		$("#tagpanel"+i).css("display","none");
	}
	$("#tagitem"+idx).attr("className","tag_headx2_open");
...
function iFrameHeight(idx){
    var ifm = document.getElementById("tagframe"+idx);
	var subWeb = document.frames ? document.frames["tagframe"+idx].document : ifm.contentDocument;   
...
<c:set var="gnidcount" value="1"/><c:forEach items="${gnid}" var="current">
			<td class="tag_headx2_close" align="center" onClick="changepanel(<c:out value='${gnidcount}'/>)" id="tagitem<c:out value='${gnidcount}'/>"><c:out value='${current.mc}'/></td>
...
<c:set var="gnidcount" value="1"/><c:forEach items="${gnid}" var="current">
	<table border="0" cellspacing="0" cellpadding="0" class="tag_body_table" id="tagpanel<c:out value='${gnidcount}'/>" style="display:none;">
		<tr>
			<td><iframe id="tagframe<c:out value='${gnidcount}'/>" onChange="iFrameHeight(<c:out value='${gnidcount}'/>)" src="" width="100%" marginwidth="0" marginheight="0" align="top" scrolling="yes" frameborder="0"></iframe></td>
		</tr>
	</table>
<c:set var="gnidcount" value="${gnidcount+1}"/>
</c:forEach>
