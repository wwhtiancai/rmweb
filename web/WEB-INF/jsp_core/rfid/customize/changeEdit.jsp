...
<script language="JavaScript" type="text/javascript">
    ...
    $(function () {
        Edit_Customize_Task_Change_NS.init();
    });

    var Edit_Customize_Task_Change_NS = {

        log: "",

        sendCommand: function (command, handler, errorHandler) {
            var result = Edit_Customize_Task_Change_NS.customizeConsole.sendMessage(command);
            if (result == "0") {
                Edit_Customize_Task_Change_NS.getResponse(1, handler, errorHandler);
                return true;
            } else {
                return false;
            }
        ...
        getResponse: function (times, handler, errorHandler) {
            if (times > 50) {
                ...
                var response = Edit_Customize_Task_Change_NS.customizeConsole.getRecvMessage();
                if (response) {
                    ...
                    setTimeout("Edit_Customize_Task_Change_NS.getResponse(" + (times + 1) + ", " + handler + "," + errorHandler + ")", 100);
                }
            }
        },
        ...
        init: function () {
            ...
            Edit_Customize_Task_Change_NS.customizeConsole = document.getElementById("customizeConsole");
            try {
            ...
            <c:if test="${task != null && task.zt == 1}">
            Edit_Customize_Task_Change_NS.fetchDeviceVersion();
            </c:if>
            ...
            $("#fzjg").dblclick(function () {
                if ($(".fzjg-select").length > 0) {
                    ...
                    if (!Edit_Customize_Task_Change_NS.bdfzjg) {
                        Edit_Customize_Task_Change_NS.bdfzjg = $("#bdfzjg").val().split(",");
                    }
                    var fzjgSelector = "<div class='fzjg-select'><ul>";
                    var existsFzjg = {};
                    $(Edit_Customize_Task_Change_NS.bdfzjg).each(function (index, element) {
            ...
            $("#requestBtn").click(function () {
                Edit_Customize_Task_Change_NS.query();
            });

            $("#customizeBtn").click(function () {
                Edit_Customize_Task_Change_NS.customize();
            });

            $("#submitBtn").click(function () {
                Edit_Customize_Task_Change_NS.submit();
            });

            $("#cancelBtn").click(function () {
                Edit_Customize_Task_Change_NS.cancel();
            });
            ...
            $("#debugBtn").click(function() {
                Tools_NS.showWarningDialog(Edit_Customize_Task_Change_NS.log);
            });
        ...
        checkDeviceStatus: function () {
            ...
            Edit_Customize_Task_Change_NS.sendCommand("1004:${cert}",
                    function (response) {
                        Edit_Customize_Task_Change_NS.SAMCert = response["SAMCert"];
                        Edit_Customize_Task_Change_NS.connected = response["ConnectStatus"] == "00";
                        if (Edit_Customize_Task_Change_NS.connected) {
                            ...
                            Tools_NS.showWarningDialog("配置设备异常，请确认设备串口是否已连接", function () {
                                Edit_Customize_Task_Change_NS.checkDeviceStatus();
                            });
                        }
                    }, function (error) {
        ...
        fetchDeviceVersion: function () {
            ...
            Edit_Customize_Task_Change_NS.sendCommand("1005", function (response) {
        ...
        submit: function () {
            $.post("/rmweb/customize-task.frm?method=change", {
                kh: Edit_Customize_Task_Change_NS.kh,
                ykh: Edit_Customize_Task_Change_NS.ykh,
                ...
                if (data && data["resultId"] == "00") {
                    Tools_NS.showConfirmDialog({
                        id: 'Success',
                        title: '创建成功 ',
                        message: '换领业务需要审核，请待业务审核通过后再进行签注',
                        width: 400,
                        cancellable: false,
                        onConfirm: function () {
                            window.close();
        ...
        customize: function () {
            Edit_Customize_Task_Change_NS.log = "C:s1:" + new Date().getTime() + "|";
            if (!Edit_Customize_Task_Change_NS.SAMCert) {
                alert("请确保读写器已正确连接");
                return;
            }

            Tools_NS.showLoading("读取电子标识中，请稍等...");
            Edit_Customize_Task_Change_NS.tid = null;
            Edit_Customize_Task_Change_NS.data = null;
            Edit_Customize_Task_Change_NS.mode = 3;
            Edit_Customize_Task_Change_NS.checkCard(1);

        },

        query: function () {
            if ($("#kh").val().length > 0 && $("#kh").val().length != 12) {
                Tools_NS.showWarningDialog("请输入正确的换领电子标识卡号");
                return;
            }
            ...
            Edit_Customize_Task_Change_NS.kh = $("#kh").val();
            Edit_Customize_Task_Change_NS.hphm = $("#fzjg").val() + $("#hphm").val();
            Edit_Customize_Task_Change_NS.hpzl = $("#hpzl").val();
            Edit_Customize_Task_Change_NS.fzjg = $("#fzjg").val();
            Edit_Customize_Task_Change_NS.ykh = $("#ykh").val();
            $.get("/rmweb/be/customize-task.rfid?method=fetch-change-info", {
                kh: Edit_Customize_Task_Change_NS.kh,
                ykh: Edit_Customize_Task_Change_NS.ykh,
                hphm: encodeURI(Edit_Customize_Task_Change_NS.hphm.toUpperCase()),
                hpzl: Edit_Customize_Task_Change_NS.hpzl,
                fzjg: encodeURI(Edit_Customize_Task_Change_NS.fzjg.toUpperCase()),
                _t: Date.parse(new Date())
            }, function (data) {
                if (data && data["resultId"] == "00") {
                    $("#oKh").text(data["oldEri"]["kh"]);
                    $("#oTid").text(data["oldEri"]["tid"]);
                    Edit_Customize_Task_Change_NS.ykh = data["oldEri"]["kh"];
                    if (data["newEri"]) {
                        $("#nKh").text(data["newEri"]["kh"]);
                        $("#nTid").text(data["newEri"]["tid"]);
                        Edit_Customize_Task_Change_NS.kh = data["newEri"]["kh"];
                    }
        ...
        checkCardStatus: function () {
            if (!Edit_Customize_Task_Change_NS.tid) {
                Tools_NS.showConfirmDialog({
                    id: 'queryDialog',
                    title: '提示 ',
            ...
            $.get("/rmweb/be/customize-task.rfid?method=check-card-status",
                    {
                        tid: Edit_Customize_Task_Change_NS.tid,
                        ywlx: "${ywlx}",
                        xh: $("#xh").val(),
                        _t: Date.parse(new Date())
                    }, function (data) {
                        if (data && data["resultId"] == "00") {
                            ...
                            $("#submitBtn").enable().focus();
                            Edit_Customize_Task_Change_NS.readCard();
                            ...
                            if (data["errorCode"] == "04") {
                                Tools_NS.showConfirmDialog({
                                    ...
                                    onCancel: 'void()',
                                    onConfirm: function () {
                                        Edit_Customize_Task_Change_NS.resetTask();
                                        $.unblockUI();
                                    }
                                });
                            } else {
                                Tools_NS.showWarningDialog(data["resultMsg"], function () {
                                    Edit_Customize_Task_Change_NS.clearTask();
                                });
                            }
        ...
        recheck: function() {
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s4:" + new Date().getTime() + "|";
            if (!Edit_Customize_Task_Change_NS.tid) {
                Tools_NS.showConfirmDialog({
                    id: 'queryDialog',
                    title: '提示 ',
                ...
                return;
            } else if ("${newEri.tid}".length > 0 && Edit_Customize_Task_Change_NS.tid != "${newEri.tid}") {
                Tools_NS.showConfirmDialog({
                    id: 'queryDialog',
                    title: '提示 ',
                    message: '当前使用的电子标识（' + Edit_Customize_Task_Change_NS.kh + '）与申请进行更改业务的电子标识（${eri.kh}）不一致<br/>' +
                    '请更新正确的电子标识后重试',
                    width: 400,
                    onCancel: 'void()',
                    onConfirm: function () {
            ...
            $.get("/rmweb/be/customize-task.rfid?method=check-card-status",
                    {
                        ...
                        Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s5:" + new Date().getTime() + "|";
                        if (data && data["resultId"] == "00") {
                            Edit_Customize_Task_Change_NS.fetchData();
                        } else if (data && data["resultId"] == "99") {
                            if (data["errorCode"] == "04") {
                                Tools_NS.showConfirmDialog({
                                    id: 'resetTaskDialog',
        ...
        fetchData: function () {
            ...
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s6:" + new Date().getTime() + "|";
            $.get("/rmweb/be/customize-task.rfid?method=fetch-data",
                    {
                        xh: $("#xh").val(),
                        tid: Edit_Customize_Task_Change_NS.tid,
                        cert: Edit_Customize_Task_Change_NS.SAMCert,
                        ywlx: "${ywlx}",
                        _t: Date.parse(new Date())
                    }, function (data) {
                        Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s7:" + new Date().getTime() + "|";
                        if (data && data["resultId"] == "00") {
                            $("#rLsh").val(data["CustomizeTask"]["lsh"]);
                            Edit_Customize_Task_Change_NS.data = data["info"]["frame"] + data["info"]["sign"];
                            Edit_Customize_Task_Change_NS.writeCard();
                        } else if (data && data["resultId"] == "99") {
                            if (data["errorCode"] == "04") {
                                Tools_NS.showConfirmDialog({
                                    id: 'resetTaskDialog',
                                ...
                                Tools_NS.showWarningDialog(data["resultMsg"], function () {
                                    ...
                                    $("#hpzl").val("");
                                    $("#lsh").val("");
                                    Edit_Customize_Task_Change_NS.unfreeze();
                                });
                            }
                        } else if (data && data["resultId"] == "98") {
                            Tools_NS.showWarningDialog(data["resultMsg"], function () {
        ...
        readCard: function () {
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s6:" + new Date().getTime() + "|";
            Edit_Customize_Task_Change_NS.sendCommand("1003", function (response) {
                Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s7:" + new Date().getTime() + "|";
                if (response["HbReadSpecResult"] == "00") {
                    Edit_Customize_Task_Change_NS.changeCardStatus(1, "读取成功");
                    ...
                    if (cardData["zt"] == 0) {
                        Tools_NS.showConfirmDialog({
                            ...
                            onConfirm: function () {
                                ...
                                Edit_Customize_Task_Change_NS.tid = null;
                                Edit_Customize_Task_Change_NS.data = null;
                                Edit_Customize_Task_Change_NS.mode = 1;
                                Edit_Customize_Task_Change_NS.checkCard(1);
                                $.unblockUI();
                            }
                        });
                    } else {
                        Edit_Customize_Task_Change_NS.setCardData(cardData);
                        $.unblockUI();
                    }
                } else {
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "读取失败");
                }
            }, function (error) {
                Edit_Customize_Task_Change_NS.changeCardStatus(2, "读取异常(" + error + ")");
            });
        },

        setCardData: function (cardData) {
            Edit_Customize_Task_Change_NS.clearCardData();
        ...
        writeCard: function () {
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s8:" + new Date().getTime() + "|";
            if (Edit_Customize_Task_Change_NS.data) {
                Tools_NS.showLoading("正在进行签注，请稍等...");
                $("#debugInfo").text("正在签注...");
                Edit_Customize_Task_Change_NS.sendCommand("1002:" + Edit_Customize_Task_Change_NS.data, function (response) {
                    var result = 0;
                    var reason = "";
                    Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s9:" + new Date().getTime() + "|";
                    if (response["HbPrivateWriteSpecResult"] == "00") {
                        if (response["HbReadSpecResult"] == "00") {
                            var checkData = ERI_NS.parse(response["HbReadSpecResultData"]);
                            if (Edit_Customize_Task_Change_NS.checkContent(checkData)) {
                                result = 1;
                            } else {
                    ...
                    } else if (response["Tid"] != Edit_Customize_Task_Change_NS.tid) {
                        $("#debugInfo").text("请求的数据与当前要签注的卡不匹配");
                        reason = "签注失败，数据TID与卡不匹配";
                    }
                    Edit_Customize_Task_Change_NS.uploadWriteResult(result, reason);
                }, function (error) {
        ...
        uploadWriteResult: function (result, reason) {
            Tools_NS.showLoading("上传签注结果【" + reason + "】");
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s10:" + new Date().getTime() + "|";
            $.get("/rmweb/be/eri.rfid?method=customize-result",
                    {xh: $("#xh").val(), result: result, sbyy: encodeURI(reason), _t: Date.parse(new Date())},
                    function (data) {
                        Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s11:" + new Date().getTime() + "|";
                        if (data && data["resultId"] == "00") {
                            if (result == 1) {
                                $("#debugInfo").text("签注成功");
                                $("#customizeBtn").attr("disabled", "disabled");
                                ...
                                Tools_NS.showWarningDialog("上传签注结果失败【" + data["resultMsg"] + "】");
                            }
                        } else {
                            Tools_NS.showConfirmDialog({
        ...
        checkCard: function (mode) {
            $("#nTid").text("");
            $("#nKh").text("");
            Edit_Customize_Task_Change_NS.changeCardStatus(0, "盘点中...");
            Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s2:" + new Date().getTime() + "|";
            Edit_Customize_Task_Change_NS.sendCommand("1001:" + (mode ? mode : "0"), function (response) {
                Edit_Customize_Task_Change_NS.log = Edit_Customize_Task_Change_NS.log + "s3:" + new Date().getTime() + "|";
                if (response["SelectSpecResult"] == "00") {
                    Edit_Customize_Task_Change_NS.tid = response["Tid"];
                    $("#nTid").text(response["Tid"]);
                    if (response["SelectSpecResultData"]) {
                        var data = ERI_NS.parse(response["SelectSpecResultData"]);
                        $("#nKh").text(data["kh"]);
                    }
                    Edit_Customize_Task_Change_NS.changeCardStatus(1, "盘点成功");
                    Edit_Customize_Task_Change_NS.doAfterChecked();
                } else if (response["SelectSpecResult"] == "02") {
                    $("#nTid").text("");
                    $("#nKh").text("");
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "盘点失败");
                    Tools_NS.showConfirmDialog({
                        id: 'noCardDialog',
                        title: '盘点失败',
                    ...
                    $("#nTid").text("");
                    $("#nKh").text("");
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "盘点成功，数据获取失败");
                    Tools_NS.showConfirmDialog({
                        id: 'noCardDialog',
                        title: '盘点失败',
                    ...
                    $("#nTid").text("");
                    $("#nKh").text("");
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "盘点异常");
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "盘点成功，数据获取失败");
                    Tools_NS.showConfirmDialog({
                        id: 'noCardDialog',
                        title: '盘点异常',
                ...
                if (error == "NO_RESPONSE") {
                    Edit_Customize_Task_Change_NS.changeCardStatus(2, "无响应,请确认是否已正确放置标识");
                    Tools_NS.showConfirmDialog({
                        id: 'noCardDialog',
                        title: '盘点失败',
                        message: '盘点失败，请确认已正确放置标识后，点“确定”重试。',
                    ...
                    Edit_Customize_Task_Change_NS.errorHandleAfterChecked(error);
                }
            });
        },
        ...
        doAfterChecked: function () {
            if (Edit_Customize_Task_Change_NS.mode == 1) {
                //读卡
                Edit_Customize_Task_Change_NS.readCard();
            } else if (Edit_Customize_Task_Change_NS.mode == 2) {
                //写卡
                Edit_Customize_Task_Change_NS.checkCardStatus();
            } else if (Edit_Customize_Task_Change_NS.mode == 3) {
                Edit_Customize_Task_Change_NS.recheck();
            }
        },
