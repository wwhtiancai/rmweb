...
function send_request(url, processName, isLoad) {
	var spath = url;
	if (isLoad) {
		postXmlHttp(spath, processName, 'load()',true);
	} else {
		postXmlHttp(spath, processName, '',true);
	}
}
function send_request_tb(url, processName, isLoad) {
	var spath = url;
	if (isLoad) {
		postXmlHttp(spath, processName, 'load()',false);
	} else {
		postXmlHttp(spath, processName, '',false);
	}
}
//-------------------------------
//  功能介绍：隐藏提示DIV层
...
function postXmlHttp(submitUrl, callbackFunc, loadFunc,bSfyb) {
	...
	_xmlHttpRequestObj.open('POST', submitUrl, bSfyb);
	_xmlHttpRequestObj.setRequestHeader("Content-Type","text/xml;charset=GBK");
