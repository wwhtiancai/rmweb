        ...
        titleBar.onmousedown = function(e) {
            ...
            document.onmouseup = function() {
                    ...
                    dashedElement.parentNode.removeChild(dashedElement);
								...
								send_request_tb("desktop.frm?method=position&xy="+setposition(getposition()),"returndesktopposition()",false);	
            };
        }
    },
...
function getposition(){
	...
	for (i = 0; i < dragTbl.rows[0].cells.length; i++) {
		var tmpStr = "";
		for (j = 0; j < dragTbl.rows[0].cells[i].getElementsByTagName("DIV").length; j++) {
			if(dragTbl.rows[0].cells[i].getElementsByTagName("DIV")[j].id.indexOf("dragDiv")!=-1){
				tmpStr += dragTbl.rows[0].cells[i].getElementsByTagName("DIV")[j].id + ",";
			}
		}
		if(tmpStr.length>1)
...
function setposition(s){
	...
	for(var i=0;i<temp.length;i++){
		if(temp[i].length<1)
			...
			r+=temp[i].replace("dragDiv","")+":"+(i*2+1)+";";
	}
	temp=tmp[1].split(",");
	for(var i=0;i<temp.length;i++){
		if(temp[i].length<1)
			...
			r+=temp[i].replace("dragDiv","")+":"+((i+1)*2)+";";
	}
	if(r.length>1)
		r=r.substr(0,r.length-1);
	return r;
}
function returndesktopposition(){
	var r=_xmlHttpRequestObj.responseText;
	if(r<1)
		document.getElementById("messagesbox").innerHTML="模块布局设置失败 ...";
}