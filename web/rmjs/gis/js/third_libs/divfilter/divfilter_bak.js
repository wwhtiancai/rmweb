...
(function($) { 
	 ...
	 */
	function createListBtn(target,val,type){
	 ...
	 *创建列表 
	 */
	function createList(target,data,type,isCheck){
		if(type==1){
	...
	function createSelWin(target,evt){
		...
		//left="10px";
		divWin.appendTo($(document.body)).addClass("divfilter-win").css({"top":top,"left":left});
		...
		createList(divWin,self.data("data"),2,false);
	...
	function createNoSelBtn(target){
		...
		var content=self.find(".selected-c");
		content.children().remove();
		var btnChoose=createListBtn(self,{text:"请选择"},-1);
		btnChoose.css({
			cursor:"hand"
