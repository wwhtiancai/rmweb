   ...
   $.fn.tooltip = function(settings) {
     ...
     function _show(target_elm) {
       ...
       if (is_top_left) {
    	   position = { 
    	   start : { 
    		   left : target_elm_position.left - $(target_elm).outerWidth() - config.animation_distance-25,
    		   top  : target_elm_position.top + ($(target_elm).outerHeight() / 2) - config.arrow_top_offset+10
    	   },
    	   end : {
    		   left : target_elm_position.left - $(target_elm).outerWidth()-25,
    		   top  : target_elm_position.top + ($(target_elm).outerHeight() / 2) - config.arrow_top_offset+10
    	   },
    	   arrow_class : "div.master_arrow"
    	   }
       }
       // coming from the top right
         ...
         position = { 
           start : { 
             left : target_elm_position.left + $(target_elm).outerWidth() + config.animation_distance-24,
             top  : target_elm_position.top + ($(target_elm).outerHeight() / 2) - config.arrow_top_offset
           },
           end : {
             left : target_elm_position.left + $(target_elm).outerWidth()-24,
             top  : target_elm_position.top + ($(target_elm).outerHeight() / 2) - config.arrow_top_offset
           },
     ...
     function _create(content_elm) {
       ...
       return $("<div class='jquery-gdakram-tooltip'>\
         ...
         <div class='master_arrow arrow'></div>\
